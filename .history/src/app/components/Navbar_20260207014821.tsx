...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 px-4 py-3 md:px-10 md:h-[80px] flex items-center justify-between bg-transparent">
      {/* Logo */}
      ...
      {/* Desktop Links */}
      <div className="hidden md:flex flex-1 justify-center items-center gap-10 bg-gray-200/40 backdrop-blur-[25px] rounded-2xl px-8 py-3 max-w-[900px]">
        {navLinks.map(link => (
          <Link
            key={link.key}
            href={link.href}
            className="flex items-center gap-1 font-semibold text-white hover:text-gray-200 transition"
          >
            {link.label}
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <button
            ...
            className="w-[46px] h-[24px] flex items-center justify-center font-semibold text-white"
          >
            {locale === 'ar' ? 'EN' : 'AR'}
          </button>
        </div>
      ...
      <div className="flex md:hidden items-center gap-4">
        ...
        >
          <span
            className={`block w-5 h-[2px] bg-white rounded transition-transform ${open ? 'rotate-45 translate-y-2' : ''
              }`}
          ></span>
          <span
            className={`block w-5 h-[2px] bg-white rounded transition-opacity ${open ? 'opacity-0' : 'opacity-100'
              }`}
          ></span>
          <span
            className={`block w-5 h-[2px] bg-white rounded transition-transform ${open ? '-rotate-45 -translate-y-2' : ''
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="absolute top-full left-4 right-4 mt-2 p-4 bg-gray-700/70 backdrop-blur-sm rounded-xl flex flex-col gap-3 md:hidden border border-white z-40">
          {navLinks.map(link => (
            <Link
              key={link.key}
              href={link.href}
