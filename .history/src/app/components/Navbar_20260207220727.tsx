...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-4 left-4 right-4 z-50 flex items-center justify-between px-5 py-3 md:px-10 md:py-5 bg-[#C1C2C266] backdrop-blur-[50px] rounded-2xl">
      ...
      {/* Desktop Navbar */}
      <div className="hidden md:flex flex-1 items-center justify-between bg-[#C1C2C266] backdrop-blur-[50px] rounded-2xl px-8 py-3 md:py-5 gap-6">
        {/* Links */}
          ...
          {navLinks.map(link => (
            ...
            >
              ...
              <span className="pointer-events-none absolute w-[56px] h-[13px] left-1/2 -translate-x-1/2 -bottom-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <svg width="57" height="13" viewBox="0 0 57 13" fill="none">
        ...
        <div className="flex items-center gap-4">
          <button className="w-5 h-5 text-white">🔍</button>
          <button onClick={switchLanguage} className="flex items-center gap-1 text-white font-medium">
            {locale === 'ar' ? 'EN' : 'AR'}
          </button>
        </div>
      </div>
      ...
      {open && (
        <div className="absolute top-full left-4 right-4 mt-2 p-4 bg-gray-700/80 backdrop-blur-sm rounded-xl flex flex-col gap-3 md:hidden z-40">
          {navLinks.map(link => (
            <Link key={link.key} href={link.href} onClick={() => setOpen(false)} className="text-white font-medium">
              {link.label}
            </Link>
          ))}
