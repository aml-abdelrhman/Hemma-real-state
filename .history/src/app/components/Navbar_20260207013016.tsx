...
const navLinks = [
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-transparent px-4  md:px-10 md:h-[80px] py-3 flex items-center justify-between">
      {/* Logo */}
      ...
      {/* Desktop Links */}
      <div className="hidden md:flex items-center gap-10 flex-1 justify-center bg-gray-200/40 backdrop-blur-[25px] rounded-2xl px-8 py-3">
        {navLinks.map((link) => (
          <Link
            key={link.key}
            href={link.href}
            className="flex items-center gap-2 font-semibold text-black hover:text-gray-700 transition"
          >
            {link.label}
